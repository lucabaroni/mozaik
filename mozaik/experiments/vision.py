...
class MeasureImagesSequence(VisualExperiment):
    ...
    def __init__(self, model, parameters):
        VisualExperiment.__init__(self, model, parameters)
        stimulus_duration = self.parameters.num_images_per_stimulus * (
            self.parameters.time_per_image + self.parameters.time_per_blank
        )
        stim_dir = os.path.join(
            mozaik.controller.Global.root_directory, "ImagesStimuli"
        )
        if not os.path.exists(stim_dir):
            os.makedirs(stim_dir)
        imgs_locations = [
            os.path.join(self.parameters.images_folder, f)
            for f in os.listdir(self.parameters.images_folder)
        ]
        imgs_locations.sort()
        imgs_locations = imgs_locations[
            self.parameters.num_skipped_images : self.parameters.num_skipped_images
            + self.parameters.num_images
        ]
        num_stimuli = int(
            numpy.ceil(
                self.parameters.num_images / self.parameters.num_images_per_stimulus
            )
        )
        ...
        for k in range(0, self.parameters.num_trials):
            for i in range(0, num_stimuli):
                ...
                filename = os.path.join(
                    stim_dir, "trial_" + str(k) + "_stimulus_" + str(i) + ".pickle"
                )
        ...
        for k in range(0, self.parameters.num_trials):
            ...
            self.stimuli.append(
                topo.SparseNoise(
                    ...
                    * (self.parameters.time_per_image + self.parameters.blank_time),
...
class MeasureSparseWithCurrentInjection(VisualExperiment):
    ...
    def __init__(self, model, parameters):
        ...
        for k in xrange(0, self.parameters.num_trials):
            ...
            d[self.parameters.stimulation_sheet] = [
                Depolarization(model.sheets[self.parameters.stimulation_sheet], p)
            ]
    ...
    def __init__(self, model, parameters):
        ...
        for s in self.parameters.intensities:
            for i in range(self.parameters.num_orientations):
                ...
                p.stimulating_signal_parameters.orientation = (
                    ParameterWithUnitsAndPeriod(
